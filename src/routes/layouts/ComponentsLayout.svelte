<script lang="ts">
	...
	$effect(() => {
		...
		for (const key in posts) {
			const paths = posts[key].map((item: any) =>
				key === 'icons' || key === 'illustrations'
					? `/${key}${item.path}`
					: `/docs/${key}${item.path}`
			);
	...
	let labelClass = '';
	let mainClass =
		'fixed inset-0 z-40 lg:z-39 flex-none h-full w-64 lg:static lg:h-auto border-e border-gray-200 dark:border-gray-600 lg:overflow-y-visible lg:pt-0 lg:block bg-white dark:bg-gray-900';
	let nonActiveClass =
	...
	let activeClass =
		'text-sm relative font-medium cursor-default bg-transparent dark:bg-transparent hover:bg-transparent dark:hover:bg-transparent text-primary-700 dark:text-primary-700';
	let triggerClass =
	...
	let contentClass =
		'overflow-y-auto px-4 pt-20 lg:pt-4 h-full scrolling-touch max-w-2xs lg:h-[calc(100vh-8rem)] lg:block lg:me-0 lg:sticky top-20 bg-white dark:bg-gray-900';
	...
	>
		...
		<SidebarGroup>
			...
			<SidebarItem
				...
				classes={{
					label:
						'ms-3 w-full text-sm font-semibold tracking-wide uppercase hover:text-primary-700 dark:hover:text-primary-600 text-gray-700 dark:text-gray-200'
				}}
			></SidebarItem>
		</SidebarGroup>
		<!-- /SidebarWrapper -->
