<script lang="ts">
	...
	const gitHub = new URL('https://github.com/themesberg/s-ui/blob/main/src/routes/');
	...
	function syncIframeTheme(): void {
		...
		if (themeLink) {
			let iframeThemeLink = iframe.contentDocument.querySelector(
				'#iframe-dynamic-theme-css'
		...
		if (fontLink) {
			let iframeFontLink = iframe.contentDocument.querySelector(
				'#iframe-dynamic-theme-font'
			) as HTMLLinkElement | null;
			if (!iframeFontLink) {
			...
			<div class="grid {meta.hideResponsiveButtons ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-3'}">
				{#if path}
					<Button
						size="xs"
						color="alternative"
						class="w-fit gap-2 hover:text-primary-600 dark:bg-gray-900"
						href={'' + path}
						target="_blank"
						rel="noreferrer"
					>
					...
					<div class="flex justify-end">
						{#if browserSupport}
							<button
								...
								type="button"
								class="copy-to-clipboard-button flex items-center border-s border-gray-200 bg-gray-100 px-3 py-2 text-xs font-medium text-gray-600 hover:text-primary-700 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-white"
							>
								<svg
									class="me-2 h-4 w-4"
									fill="none"
				...
				<div class="relative">
					...
					{#if showExpandButton && !expand}
						<button
							...
							data-expand-code=""
							type="button"
							class="absolute start-0 bottom-0 w-full border-t border-gray-200 bg-gray-100 px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
						>
							Expand code
						</button>
					{/if}
