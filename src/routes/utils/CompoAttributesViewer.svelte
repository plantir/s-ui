<script lang="ts">
	...
	let {
		...
		components,
		plugin
	}: { dirName: string; components: string; plugin: string } = $props();
	...
	onMount(async () => {
		try {
			...
			const [chartResponse, tableResponse] = await Promise.all([
				fetch(
					'https://raw.githubusercontent.com/shinokada/s-ui-plugins/main/apps/s-ui-chart/src/routes/component-data/Chart.json'
				),
				fetch(
					'https://raw.githubusercontent.com/shinokada/s-ui-plugins/main/apps/s-ui-datatable/src/routes/component-data/Table.json'
				)
			]);
