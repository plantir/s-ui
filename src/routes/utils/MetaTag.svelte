<script lang="ts">
	...
	let imgsrc = $derived(
		`https://s-ui.com/og?title=${encodeURIComponent(breadcrumb_title)}&package=${encodeURIComponent(pkg)}`
	);
...
<MetaTags
	...
	openGraph={{
		type: 'website',
		url: `https://s-ui-blocks.codewithshin.com/${dirstring}${breadcrumb ? `/${breadcrumb}` : ''}`,
		title: `${title}`,
