...
<div class="container mx-auto max-w-5xl px-4 py-8">
	<Heading tag="h1" class="mb-6 text-3xl font-bold"
		>Virtual Masonry Image Gallery (50 images)</Heading
	>
	<P>
		A virtualized Svelte component rendering an efficient masonry/Pinterest layout. It calculates
		item positions and uses windowing (overscan) to display only visible items from a large dataset,
		...
		>
			{#snippet children(image, _index)}
				<div class="group relative h-full">
					...
					/>
					<div
						class="absolute right-0 bottom-0 left-0 rounded-b-lg bg-gradient-to-t from-black/70 to-transparent p-3 text-white opacity-0 transition-opacity group-hover:opacity-100"
					>
