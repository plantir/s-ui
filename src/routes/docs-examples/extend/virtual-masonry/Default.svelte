<script lang="ts">
  import { Spinner, VirtualMasonry, Heading } from "$lib";
...
<div class="container mx-auto px-4 py-8">
  <Heading tag="h1" class="mb-6 text-3xl font-bold">Virtual Masonry Image Gallery (limit: 50)</Heading>
  ...
  {:else}
    <VirtualMasonry items={data.images} columns={3} gap={16} height={800} overscan={200} getItemHeight={getImageHeight} classes={{ item: "p-2" }}>
      {#snippet children(image, _index)}
        <div class="group relative h-full">
          <img src={image.url} alt={image.alt} class="h-full w-full rounded-lg object-cover shadow-md" loading="lazy" />
