<script lang="ts">
	...
	const mainMarginClass = $derived(
		position === 'left' ? 'ml-64' : position === 'right' ? 'mr-64' : ''
	);
</script>

<div class="{isFlex ? 'flex' : ''} bg-neutral-primary-soft pb-32">
	...
	<main class="container mx-auto px-4 pb-8 {mainMarginClass}">
		...
		<div class="my-4 flex p-2">
			...
			<div class="flex gap-3">
				<Radio bind:group={position} value="top" onchange={() => handlePositionChange('top')}
					>Top</Radio
				>
				<Radio bind:group={position} value="left" onchange={() => handlePositionChange('left')}
					>Left</Radio
				>
				<Radio bind:group={position} value="right" onchange={() => handlePositionChange('right')}
					>Right</Radio
				>
			</div>
		</div>
		...
		<section id="introduction" class="p-4">
			...
			<P class="mb-4 text-lg">
				A navigation component that tracks scroll position and highlights the currently visible
				section. Supports smooth scrolling, sticky positioning, custom scroll containers, offset
				handling, and active state callbacks for building interactive page or documentation
				navigation.
			</P>
		</section>
		...
		<section id="installation" class="border-t border-default px-4 py-8">
			...
			<HighlightSvelte
				code={exampleModules['./md/installation.md'] as string}
				langtag
				--langtag-color="orange"
				class="mb-4"
			/>
			<P>This installs `s-ui` (Svelte components) and `flowbite` as development dependencies.</P>
		</section>
		...
		<section id="basic-usage" class="px-4 pb-16">
			...
			<P class="mb-6 text-body"
				>The simplest way to use ScrollSpy is to provide an array of navigation items:</P
			>
			<HighlightSvelte
				code={exampleModules['./md/usage.md'] as string}
				langtag
				--langtag-color="orange"
				class="mb-4"
			/>
		...
		<section id="features" class="px-4 pb-16">
			...
			<HighlightSvelte
				code={exampleModules['./md/position.md'] as string}
				langtag
				--langtag-color="orange"
				class="mb-4"
			/>
			...
			<HighlightSvelte
				code={exampleModules['./md/offset.md'] as string}
				langtag
				--langtag-color="orange"
				class="mb-4"
			/>
			...
			<HighlightSvelte
				code={exampleModules['./md/style.md'] as string}
				langtag
				--langtag-color="orange"
				class="mb-4"
			/>
		...
		<section id="accessibility" class="px-4 pb-16">
			...
			<div
				class="mt-8 rounded-lg border border-green-200 bg-green-50 p-6 dark:border-green-800 dark:bg-green-900/20"
			>
