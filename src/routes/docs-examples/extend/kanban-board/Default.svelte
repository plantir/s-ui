<script lang="ts">
	...
	function handleAddCard(col: KanbanColumnType) {
		...
		columns = columns.map((column) =>
			column.id === col.id ? { ...column, cards: [...column.cards, newCard] } : column
		);
...
<div class="bg-neutral-tertiary py-4 md:py-8">
	<div class="mx-auto max-w-7xl px-2 sm:px-4">
		<div class="mb-4 md:mb-6">
			...
			<P class="mt-1 text-sm text-body md:mt-2 md:text-base"
				>Drag cards between columns to update their status</P
			>
		</div>
