<script lang="ts">
  import { type KanbanCardType, type KanbanColumnType, KanbanBoard, Heading, P } from "$lib";
...
<div class="bg-neutral-tertiary py-4 md:py-8">
  <div class="mx-auto max-w-7xl px-2 sm:px-4">
    <div class="mb-4 md:mb-6">
      ...
      <P class="text-body mt-1 text-sm md:mt-2 md:text-base">Drag cards between columns to update their status</P>
    </div>
    ...
    <div class="mt-6 grid grid-cols-2 gap-3 md:mt-8 md:grid-cols-4 md:gap-4">
      {#each columns as col (col.id)}
        <div class="bg-neutral-primary-soft rounded-lg p-3 shadow-sm md:p-4">
          <div class="text-body text-xs md:text-sm">{col.title}</div>
          <div class="text-heading mt-1 text-xl font-bold md:text-2xl">{col.cards.length}</div>
        </div>
      {/each}
    </div>
  </div>
