...
<div class="space-y-6">
  ...
  <form onsubmit={handleSubmit} class="space-y-4">
    {#if currentStep === 1}
      <div class="space-y-4">
        <h3 class="text-heading text-lg font-semibold">Personal Information</h3>
        <div>
    ...
    {:else if currentStep === 2}
      <div class="space-y-4">
        <h3 class="text-heading text-lg font-semibold">Contact Details</h3>
        <div>
    ...
    {:else if currentStep === 3}
      <div class="space-y-4">
        <h3 class="text-heading text-lg font-semibold">Address Information</h3>
        <div>
    ...
    {:else if currentStep === 4}
      <div class="space-y-4">
        <h3 class="text-heading text-lg font-semibold">Review Your Information</h3>
        <div class="border-default rounded-lg border p-4">
          <dl class="space-y-2">
            <div class="flex justify-between">
              <dt class="text-body font-medium">Name:</dt>
              <dd class="text-heading">{personalInfo.firstName} {personalInfo.lastName}</dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-body font-medium">Email:</dt>
              <dd class="text-heading">{contactInfo.email}</dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-body font-medium">Phone:</dt>
              <dd class="text-heading">{contactInfo.phone}</dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-body font-medium">Address:</dt>
              <dd class="text-heading">{addressInfo.street}, {addressInfo.city} {addressInfo.zip}</dd>
            </div>
          </dl>
        </div>
        <p class="text-body text-sm">Click on any step indicator above to go back and edit your information.</p>
      </div>
    {:else if currentStep === 5}
      <div class="space-y-4 text-center">
        ...
        <h3 class="text-heading text-lg font-semibold">Registration Complete!</h3>
