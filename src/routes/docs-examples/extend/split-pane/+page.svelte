...
<div class="bg-neutral-tertiary">
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Simple two-pane horizontal layout with draggable divider. Demonstrates core resizing
			functionality with visual feedback showing percentage and pixel widths.
		</p>
			...
			<SplitPane responsive={false} onResize={handleBasicResize}>
					...
					<div class="h-full bg-blue-50 p-4">
						...
						{#if basicSizes.length > 0 && basicContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {basicSizes[0].toFixed(2)}% = {getPixels(
									basicSizes[0],
									basicContainerWidth
								)}px
							</div>
				...
				<Pane>
					<div class="h-full bg-green-50 p-4">
						...
						{#if basicSizes.length > 0 && basicContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {basicSizes[1].toFixed(2)}% = {getPixels(
									basicSizes[1],
									basicContainerWidth
								)}px
							</div>
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Two-pane horizontal layout with minSize of 200 and initialSizes of [25,75] and Sidebar
			components on the left pane.
		</p>
		{#if basicWithSidebarSizes.length > 0 && basicWithSidebarWidth > 0}
			<div class="mb-2 text-sm text-body">
				...
				<span class="ml-4"><strong>Sizes:</strong></span>
				<span class="ml-2">
					Left: {basicWithSidebarSizes[0].toFixed(1)}% ({getPixels(
						basicWithSidebarSizes[0],
						basicWithSidebarWidth
					)}px)
				</span>
				<span class="ml-3">
					Right: {basicWithSidebarSizes[1].toFixed(1)}% ({getPixels(
						basicWithSidebarSizes[1],
						basicWithSidebarWidth
					)}px)
				</span>
			</div>
			...
			>
					...
					<div class="h-full bg-blue-50 p-4">
						...
						<SidebarGroup>
							<SidebarItem label="Dashboard" href="/">
								{#snippet icon()}
									<ChartOutline
										class="inline h-5 w-5 text-body transition duration-75 group-hover:text-heading"
									/>
								{/snippet}
							</SidebarItem>
							<SidebarItem label="Kanban" classes={{ label: labelClass }} href="/">
								...
								{#snippet subtext()}
									<span
										class="ms-3 inline-flex items-center justify-center rounded-full bg-neutral-quaternary px-2 text-sm font-medium text-heading"
										>Pro</span
									>
								{/snippet}
							</SidebarItem>
							<SidebarItem label="Inbox" classes={{ label: labelClass }} href="/">
								...
								{#snippet subtext()}
									<span
										class="ms-3 inline-flex h-3 w-3 items-center justify-center rounded-full bg-brand-softer p-3 text-sm font-medium text-fg-brand"
										>3</span
									>
								{/snippet}
							</SidebarItem>
				...
				<Pane>
					<div class="h-full bg-green-50 p-4">
						<h3 class="font-semibold">Right Pane</h3>
						<p>
							This is the right pane content. Lorem ipsum dolor sit, amet consectetur adipisicing
							elit. Impedit nulla ipsum inventore nihil labore in velit dolores consequatur,
							voluptas praesentium perferendis nobis sequi culpa laboriosam natus! Dignissimos
							exercitationem vitae necessitatibus.
						</p>
					</div>
				</Pane>
			</SplitPane>
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Vertical layout with custom initial sizes and keyboard controls. Use arrow keys to adjust,
			Enter/Space to reset to equal distribution.
		</p>
		{#if verticalSizes.length > 0 && verticalContainerHeight > 0}
			<div class="mb-2 text-sm text-body">
				...
				<span class="ml-4"><strong>Sizes:</strong></span>
				<span class="ml-2">
					Top: {verticalSizes[0].toFixed(1)}% ({getPixels(
						verticalSizes[0],
						verticalContainerHeight
					)}px)
				</span>
				<span class="ml-3">
					Bottom: {verticalSizes[1].toFixed(1)}% ({getPixels(
						verticalSizes[1],
						verticalContainerHeight
					)}px)
				</span>
			...
			>
				<Pane>
					<div class="h-full bg-purple-50 p-4">
						...
						{#if verticalSizes.length > 0 && verticalContainerHeight > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Height: {verticalSizes[0].toFixed(2)}% = {getPixels(
									verticalSizes[0],
									verticalContainerHeight
								)}px
							</div>
				...
				<Pane>
					<div class="h-full bg-yellow-50 p-4">
						...
						{#if verticalSizes.length > 0 && verticalContainerHeight > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Height: {verticalSizes[1].toFixed(2)}% = {getPixels(
									verticalSizes[1],
									verticalContainerHeight
								)}px
							</div>
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Multiple panes in single container with individual dividers. Each pane can be resized
			independently while maintaining minimum size constraints.
		</p>
		{#if threePaneSizes.length > 0 && threePaneContainerWidth > 0}
			<div class="mb-2 text-sm text-body">
				...
				<span class="ml-4"><strong>Sizes:</strong></span>
				<span class="ml-2">
					Pane 1: {threePaneSizes[0].toFixed(1)}% ({getPixels(
						threePaneSizes[0],
						threePaneContainerWidth
					)}px)
				</span>
				<span class="ml-3">
					Pane 2: {threePaneSizes[1].toFixed(1)}% ({getPixels(
						threePaneSizes[1],
						threePaneContainerWidth
					)}px)
				</span>
				<span class="ml-3">
					Pane 3: {threePaneSizes[2].toFixed(1)}% ({getPixels(
						threePaneSizes[2],
						threePaneContainerWidth
					)}px)
				</span>
			...
			<SplitPane minSize={80} responsive={false} onResize={handleThreePaneResize}>
				<Pane>
					<div class="h-full bg-red-50 p-4">
						...
						{#if threePaneSizes.length > 0 && threePaneContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {threePaneSizes[0].toFixed(2)}% = {getPixels(
									threePaneSizes[0],
									threePaneContainerWidth
								)}px
							</div>
				...
				<Pane>
					<div class="h-full bg-blue-50 p-4">
						...
						{#if threePaneSizes.length > 0 && threePaneContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {threePaneSizes[1].toFixed(2)}% = {getPixels(
									threePaneSizes[1],
									threePaneContainerWidth
								)}px
							</div>
				...
				<Pane>
					<div class="h-full bg-green-50 p-4">
						...
						{#if threePaneSizes.length > 0 && threePaneContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {threePaneSizes[2].toFixed(2)}% = {getPixels(
									threePaneSizes[2],
									threePaneContainerWidth
								)}px
							</div>
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Automatically switches between horizontal and vertical layouts based on viewport width.
			Features callback tracking for size change monitoring.
		</p>
		{#if responsiveSizes.length > 0 && responsiveContainerWidth > 0}
			<div class="mb-2 text-sm text-body">
				...
				<span class="ml-4"><strong>Sizes:</strong></span>
				<span class="ml-2">
					Pane A: {responsiveSizes[0].toFixed(1)}% ({getPixels(
						responsiveSizes[0],
						responsiveContainerWidth
					)}px)
				</span>
				<span class="ml-3">
					Pane B: {responsiveSizes[1].toFixed(1)}% ({getPixels(
						responsiveSizes[1],
						responsiveContainerWidth
					)}px)
				</span>
			...
			<SplitPane responsive={true} breakpoint={768} onResize={handleResponsiveResize}>
				<Pane>
					<div class="h-full bg-indigo-50 p-4">
						...
						<p>
							Resize your browser window to see the layout change from horizontal to vertical at
							768px.
						</p>
						{#if responsiveSizes.length > 0 && responsiveContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Size: {responsiveSizes[0].toFixed(2)}% = {getPixels(
									responsiveSizes[0],
									responsiveContainerWidth
								)}px
							</div>
				...
				<Pane>
					<div class="h-full bg-pink-50 p-4">
						...
						{#if responsiveSizes.length > 0 && responsiveContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Size: {responsiveSizes[1].toFixed(2)}% = {getPixels(
									responsiveSizes[1],
									responsiveContainerWidth
								)}px
							</div>
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Complex layout combining horizontal and vertical splits. Creates sidebar with vertically-split
			main content area for advanced dashboard layouts.
		</p>
		{#if nestedSizes.length > 0 && nestedContainerWidth > 0}
			<div class="mb-2 text-sm text-body">
				...
				<span class="ml-2">
					Sidebar: {nestedSizes[0].toFixed(1)}% ({getPixels(
						nestedSizes[0],
						nestedContainerWidth
					)}px)
				</span>
				<span class="ml-3">
			...
			<SplitPane responsive={false} onResize={handleNestedResize}>
				<Pane>
					<div class="h-full bg-neutral-secondary-soft p-4">
						...
						{#if nestedSizes.length > 0 && nestedContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {nestedSizes[0].toFixed(2)}% = {getPixels(
									nestedSizes[0],
									nestedContainerWidth
								)}px
							</div>
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Demonstrates minimum size enforcement. Panes cannot be resized below 300px, preventing content
			from becoming unusable during aggressive resizing.
		</p>
		{#if constrainedSizes.length > 0 && constrainedContainerWidth > 0}
			<div class="mb-2 text-sm text-body">
				...
				<span class="ml-4"><strong>Sizes:</strong></span>
				<span class="ml-2">
					Left: {constrainedSizes[0].toFixed(1)}% ({getPixels(
						constrainedSizes[0],
						constrainedContainerWidth
					)}px)
				</span>
				<span class="ml-3">
					Right: {constrainedSizes[1].toFixed(1)}% ({getPixels(
						constrainedSizes[1],
						constrainedContainerWidth
					)}px)
				</span>
			...
			>
				<Pane>
					<div class="h-full bg-orange-50 p-4">
						...
						{#if constrainedSizes.length > 0 && constrainedContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {constrainedSizes[0].toFixed(2)}% = {getPixels(
									constrainedSizes[0],
									constrainedContainerWidth
								)}px
							</div>
				...
				<Pane>
					<div class="h-full bg-teal-50 p-4">
						...
						{#if constrainedSizes.length > 0 && constrainedContainerWidth > 0}
							<div class="mt-4 rounded border bg-neutral-primary-soft p-2 font-mono text-xs">
								Width: {constrainedSizes[1].toFixed(2)}% = {getPixels(
									constrainedSizes[1],
									constrainedContainerWidth
								)}px
							</div>
	...
	<div class="mb-8 bg-neutral-primary-soft p-8">
		...
		<p class="text-body">
			Production-ready IDE-style interface with file explorer, code editor, and terminal. Shows
			practical implementation with nested panes and dark theme.
		</p>
		{#if editorSizes.length > 0 && editorContainerWidth > 0}
			<div class="mb-2 text-sm text-body">
				...
				<span class="ml-4"><strong>Sizes:</strong></span>
				<span class="ml-2">
					Explorer: {editorSizes[0].toFixed(1)}% ({getPixels(
						editorSizes[0],
					...
					<div class="h-full overflow-auto bg-gray-900 p-4 text-white">
						...
						{#if editorSizes.length > 0 && editorContainerWidth > 0}
							<div class="mt-4 rounded border border-gray-700 bg-gray-800 p-2 font-mono text-xs">
								Width: {editorSizes[0].toFixed(2)}% = {getPixels(
									editorSizes[0],
									editorContainerWidth
								)}px
							</div>
