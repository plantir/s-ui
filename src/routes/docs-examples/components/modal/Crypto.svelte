...
<Modal title="Connect wallet" bind:open={walletModal} size="xs">
	...
	<ul class="my-4 space-y-3">
		<li>
			<a
				href="/"
				class="group flex items-center rounded-base bg-neutral-tertiary p-3 text-base font-bold text-heading hover:bg-neutral-secondary-medium hover:shadow-xs"
			>
				<MetaMask />
				<span class="ms-3 flex-1 whitespace-nowrap">MetaMask</span>
				<span
					class="ms-3 inline-flex items-center justify-center rounded-sm bg-neutral-quaternary px-2 py-0.5 text-xs font-medium text-body"
					>Popular</span
				>
			</a>
		</li>
