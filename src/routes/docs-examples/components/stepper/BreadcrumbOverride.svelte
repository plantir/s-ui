...
<Heading tag="h3" class="mb-4 text-lg font-semibold"
	>Breadcrumb Stepper with Status Override</Heading
>
...
<div class="mt-4 rounded bg-gray-50 p-4 dark:bg-gray-800">
		...
		{#each steps as step, i}
			<Li>
				...
				<Span class="font-mono">
					{step.status ??
						(i + 1 < current ? 'completed' : i + 1 === current ? 'current' : 'pending')}
				</Span>
			</Li>
		{/each}
	</List>
	...
	<P class="mt-4 text-sm text-gray-600 dark:text-gray-400"
		>Note: Step 1 is forced to "completed" and Step 4 is forced to "pending" regardless of the
		current value.</P
	>
</div>
