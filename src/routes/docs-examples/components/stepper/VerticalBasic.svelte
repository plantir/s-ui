<script lang="ts">
	import { VerticalStepper, Button, P, Heading } from "$lib/index";
...
<Heading tag="h3" class="mb-4 text-lg font-semibold"
	>Example 1: Basic clickable vertical stepper</Heading
>

<VerticalStepper {steps} bind:current onStepClick={handleStepChange} />

<Heading tag="h3" class="mt-8 mb-4 text-lg font-semibold"
	>Example 2: Non-clickable vertical stepper</Heading
>
<VerticalStepper {steps} bind:current clickable={false} />

<Heading tag="h3" class="mt-8 mb-4 text-lg font-semibold"
	>Example 3: Without checkmarks (no icon for completed steps)</Heading
>
...
<div class="mt-4 rounded bg-gray-50 p-4 dark:bg-gray-800">
	...
	<P class="mt-2 text-xs text-gray-500 dark:text-gray-400"
		>Note: current=0 means no step is active, current=1 is the first step, etc.</P
	>
</div>
