<script lang="ts">
	import { Timeline, TimelineItem, P } from "$lib/index";
	...
	{#each appointments as appointment, index}
		...
		>
			{#snippet orientationSlot()}
				<span
					class="absolute -start-3 flex h-6 w-6 items-center justify-center rounded-full ring-8 ring-buffer {appointment.status ===
					'completed'
			...
			{/snippet}
			<div class="pl-4">
				<p class="mb-2 text-base font-normal text-body">
...
<Timeline order="vertical" class="mt-8">
	<TimelineItem
		...
		datePrefix="Released on"
	>
		<p class="pl-4 text-sm text-body">This event has already happened</p>
	</TimelineItem>
	...
	<TimelineItem
		...
		datePrefix="Released on"
	>
		<p class="pl-4 text-sm text-body">This event is happening now</p>
	</TimelineItem>
	...
	<TimelineItem
		...
		datePrefix="Released on"
	>
		<p class="pl-4 text-sm text-body">This event will happen in the future</p>
	</TimelineItem>
</Timeline>
		...
		>
			{#snippet orientationSlot()}
				<div class="flex items-center">
					<div
						class="z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ring-0 ring-buffer sm:ring-8 {release.status ===
						'completed'
							? 'bg-green-200 dark:bg-green-900'
			...
			<span
				class="mt-2 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {release.status ===
				'completed'
					? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300'
					: release.status === 'in-progress'
						? 'bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-300'
...
<Timeline order="horizontal" class="mt-8">
	<TimelineItem title="Past Release" color="green" date={pastDate}>
		<p class="text-base font-normal text-body">This version was released successfully</p>
	</TimelineItem>

	<TimelineItem title="Current Development" color="orange" date={currentDate}>
		<p class="text-base font-normal text-body">This version is currently in development</p>
	</TimelineItem>

	<TimelineItem title="Future Release" color="blue" isLast={true} date={futureDate}>
		<p class="text-base font-normal text-body">This version is planned for the future</p>
	</TimelineItem>
</Timeline>
