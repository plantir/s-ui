<script lang="ts">
	...
	function handleTimeChange(
		index: number,
		isStartTime: boolean,
		event: { time: string; endTime: string; [key: string]: string }
	): void {
	...
	const timepickerClasses = {
		...
		inputClass:
			'block disabled:cursor-not-allowed disabled:opacity-50 p-1.5 sm:p-2.5 text-xs sm:text-sm border-r-0 focus:ring-0 focus:outline-none'
	};
</script>
	...
	<form onsubmit={saveAll} class="space-y-8">
		...
		<div class="space-y-2 sm:space-y-4">
			{#each sortedWorkingDays as { day, enabled, startTime, endTime }, index (day)}
				<div
					class="flex flex-col gap-2 rounded-lg bg-white p-2 shadow-sm transition-shadow hover:shadow-md dark:bg-gray-700"
				>
