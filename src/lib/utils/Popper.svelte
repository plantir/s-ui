<script lang="ts">
	...
	let arrowParams = $state<{ placement: Placement; cords: Partial<Coords> }>({
		placement: 'top',
		cords: { x: 0, y: 0 }
	});
	$effect(() => {
		...
		if (ev.target !== invoker && triggerEls.includes(ev.target as HTMLElement)) {
			...
			if (isOpen) {
				...
				popover?.dispatchEvent(
					new ToggleEvent('beforetoggle', { newState: 'open', oldState: 'open' })
				);
	...
	const [open_popover, close_popover] = createMutualDebounce(
		_open_popover,
		_close_popover,
		() => triggerDelay
	);
...
## Type
[PopperProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L2164)
## Props
@prop triggeredBy
