<script lang="ts">
	...
	import type { TimepickerProps, TimePickerOption } from '$lib/types.js';
	import Input from '$lib/input-field/Input.svelte';
		...
		{:else if type === 'range'}
			<div class={styles.rangeInputWrapper({ class: clsx(theme?.rangeInputWrapper) })}>
				...
				>
					...
					{:else}
						<svg
							class={styles.icon({ class: clsx(theme?.icon) })}
							aria-hidden="true"
							xmlns="http://www.w3.org/2000/svg"
							fill="none"
							viewBox="0 0 20 20"
					...
					<div class={styles.dropdownTimeRow({ class: clsx(theme?.dropdownTimeRow) })}>
						<div class={styles.dropdownTimeCol({ class: clsx(theme?.dropdownTimeCol) })}>
							...
							<Input
								...
								{required}
								{disabled}
								class={styles.dropdownTimeInput({
									class: clsx(theme?.dropdownTimeInput, inputClass)
								})}
								bind:value
								oninput={(e) => handleTimeChange(e)}
								onchange={(e) => handleTimeChange(e)}
...
## Type
[TimepickerProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1020)
## Props
@prop id = "time"
@prop endId = "end-time"
