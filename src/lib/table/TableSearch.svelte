<script lang="ts">
	...
	import type { TableSearchProps, TableContextType } from '$lib/types.js';
	import clsx from 'clsx';
	import { tableSearch } from './theme.js';
	...
	const themeColor = $derived(
		color === 'custom'
			? 'default'
			: (color as 'default' | 'blue' | 'green' | 'red' | 'yellow' | 'purple' | 'indigo' | 'pink')
	);

	const { root, inner, search, svgDiv, svg, input, table } = $derived(
		tableSearch({ color: themeColor, striped, hoverable })
	);

	const tableCls = $derived(table({ class: clsx(tableClass, theme?.table, className) }));

	// Handle custom color
	const finalTableClass = $derived(
		color === 'custom' && customColor ? clsx(tableCls, customColor) : tableCls
	);
	...
	<div class={inner({ class: clsx(theme?.inner, styling.inner) })}>
		...
		<div class={search({ class: clsx(theme?.search, styling.search) })}>
			<div class={svgDiv({ class: clsx(theme?.svgDiv, styling.svgDiv) })}>
				<svg
					class={svg({ class: clsx(theme?.svg, styling.svg) })}
					fill="currentColor"
					viewBox="0 0 20 20"
...
## Type
[TableSearchProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1834)
## Props
@prop children
@prop header
@prop footer
