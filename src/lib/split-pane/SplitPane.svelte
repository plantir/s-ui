<script lang="ts">
	import type { SplitPaneProps } from '$lib/types.js';
	...
	setSplitPaneContext({
		registerPane,
		getPaneStyle,
		getPaneSize: (index: number) =>
			sizes[index] ?? (registeredPanes > 0 ? 100 / registeredPanes : 0),
		shouldRenderDivider,
	...
	$effect(() => {
		...
		if (totalMinRequired > 100) {
			console.error(
				`Cannot satisfy minSize=${minSize}px for ${registeredPanes} panes in ${containerSize}px container. ` +
					`Required: ${(minSize * registeredPanes).toFixed(0)}px. Using equal distribution.`
			);
			const equal = 100 / registeredPanes;
			sizes = Array.from({ length: registeredPanes }, () => equal);
			return;
		...
		if (
			initialSizes &&
			initialSizes.length === registeredPanes &&
			sizes.length !== registeredPanes
		) {
...
<div
	bind:this={container}
	class={splitpane({ direction: currentDirection, class: clsx(theme, className) })}
>
	{@render children()}
</div>
...
## Type
[SplitPaneProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L2239)
## Props
