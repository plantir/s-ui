<script lang="ts">
	import clsx from 'clsx';
	import { navbarBrand } from './theme.js';
	import type { NavBrandProps } from '$lib/types.js';
	import { getTheme } from '$lib/theme/themeUtils';

	let { children, class: className, ...restProps }: NavBrandProps = $props();

	const theme = $derived(getTheme('navbarBrand'));
</script>

<a {...restProps} class={navbarBrand({ class: clsx(theme, className) })}>
	{@render children?.()}
</a>

<!--
@component
[Go to docs](https://s-ui.com/)
## Type
[NavBrandProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1197)
## Props
@prop children
@prop class: className
@prop ...restProps
-->
