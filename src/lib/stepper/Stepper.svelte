<script lang="ts">
	...
	import { stepper } from './theme.js';
	import type { StepperProps } from '$lib/types.js';
...
{#snippet stepContent(step: Step, status: StepStatus, index: number)}
	...
	{step.label}
	{#if step.description}
		<span class={clsx(step.descriptionClass) || 'hidden sm:ms-2 sm:inline-flex'}
				...
				<button
					...
					class={content({
						...
						class: clsx(
							theme?.content,
							classes?.content,
							'w-full cursor-pointer text-left transition-opacity hover:opacity-75'
						)
					})}
...
## Type
[StepperProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1658)
## Props
@prop steps = []
