<script lang="ts">
	import type { StepStatus, Step, VerticalStepperProps } from '$lib/types.js';
	import CheckmarkIcon from './CheckmarkIcon.svelte';
	import { verticalStepper } from './theme.js';
	...
	{#each steps as step, index (step.id)}
		...
		<li class={clsx(liClass)}>
			{#if clickable}
				<button
					type="button"
					class="w-full cursor-pointer text-left transition-opacity hover:opacity-75 {card({
						status,
						class: clsx(theme?.card, classes?.card)
					})}"
			...
			{:else}
				<div
					class={card({ status, class: clsx(theme?.card, classes?.card) })}
					aria-current={status === 'current' ? 'step' : undefined}
				>
					<div class={content({ class: clsx(theme?.content, classes?.content) })}>
						<span class="sr-only">{step.label}</span>
...
## Type
[VerticalStepperProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1710)
## Props
@prop steps = []
@prop liClass
