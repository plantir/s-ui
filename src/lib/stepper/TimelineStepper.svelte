<script lang="ts">
	import type { StepStatus, TimelineStep, TimelineStepperProps } from '$lib/types.js';
	import CheckmarkIcon from './CheckmarkIcon.svelte';
	import ProfileCardIcon from './ProfileCardIcon.svelte';
	import { timelineStepper } from './theme.js';
...
<ol class={base({ class: clsx(theme?.base, className) })} {...restProps}>
	{#each steps as step, index (step.id)}
		...
		<li
			class={item({ isLast: index === steps.length - 1, class: clsx(theme?.item, classes?.item) })}
		>
			{#if clickable}
				<button
					type="button"
			...
			{:else}
				<span
					class={circle({ status, class: clsx(theme?.circle, classes?.circle) })}
					aria-current={status === 'current' ? 'step' : undefined}
				>
					{@render stepIcon(status, step)}
				</span>
...
## Type
[TimelineStepperProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1747)
## Props
@prop steps = []
