<script lang="ts">
	...
	import { timelineItem, type TimelineVariants } from './theme.js';
	import type { TimelineItemProps, DateFormat } from '$lib/types.js';
	...
	const { base, div, defaultDiv, time, h3, svg, connector } = $derived(
		timelineItem({ order, color, isLast })
	);
...
<li {...restProps} class={base({ class: clsx(theme?.base, className ?? liClass) })}>
	...
	{#if !isLast && (order === 'vertical' || order === 'activity')}
		<div
			class={connector({ class: clsx(theme?.connector, styling.connector) })}
			aria-hidden="true"
		></div>
	{/if}
...
## Type
[TimelineItemProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1960)
## Props
@prop children
