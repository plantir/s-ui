<script lang="ts">
	...
	import { datepicker } from './theme.js';
	import {
		parse,
		isValid,
		addDays,
	...
	const getWeekdayNames = (): string[] => {
		...
		return Array.from({ length: 7 }, (_, i) =>
			addDays(referenceDate, i).toLocaleDateString(finalTranslationLocale, { weekday: 'short' })
		);
	};
	...
	const getMonthNames = (): string[] => {
		return Array.from({ length: 12 }, (_, i) =>
			new Date(2000, i, 1).toLocaleDateString(finalTranslationLocale, { month: 'short' })
		);
	};
		...
		>
			...
			{:else}
				<div class={nav({ class: clsx(classes?.nav) })}>
					...
					>
						{currentMonth.toLocaleString(finalTranslationLocale, {
							month: 'long',
							year: 'numeric'
						})}
					</Button>
...
## Type
[DatepickerProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L457)
## Props
@prop value = $bindable()
