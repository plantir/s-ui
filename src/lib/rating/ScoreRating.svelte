<script lang="ts">
	import { scoreRating } from './theme.js';
	import type { ScoreRatingProps } from '$lib/types.js';
...
<div class="mb-5 flex items-center">
	{#if headerLabel}
		...
		{/if}
		{#if headerLabel.link}
			<a href={headerLabel.link.url} class={link({ class: clsx(theme?.link, classes?.link) })}
				...
				<dl>
					...
					<dd class="mb-3 flex items-center">
						<div class="me-2 h-2.5 w-full rounded-sm bg-gray-200 dark:bg-gray-700">
							<div
								class={bar({ class: clsx(theme?.bar, classes?.bar) })}
