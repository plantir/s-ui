<script lang="ts">
	...
	import { review as reviewVariants } from './theme.js';
	import type { ReviewProps } from '$lib/types.js';
	...
	<article class={article({ class: clsx(theme?.article, styling.article) })}>
		<div>
			<div class={div({ class: clsx(theme?.div, styling.div) })}>
				<img
					class={img({ class: clsx(theme?.img, styling.img) })}
					src={review.imgSrc}
					alt={review.imgAlt}
				/>
		...
		<div class="col-span-2 mt-6 md:mt-0">
			<div class="mb-5 flex items-start">
				...
				<p
					class="inline-flex items-center rounded-sm bg-primary-700 p-1.5 text-sm font-semibold text-white"
				>
					{review.rating}
				</p>
			</div>
...
## Type
[ReviewProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1403)
## Props
@prop children
@prop address
@prop item1
