<script lang="ts" generics="T">
	import type { VirtualListProps } from '$lib/types.js';
	import { virtualList } from './theme.js';
	...
	const totalHeight = $derived.by(() =>
		items.reduce(
			(sum: number, item, i) =>
				sum + (getItemHeight ? getItemHeight(item, i) : (minItemHeight ?? 50)),
			0
		)
	);
...
>
	<div
		class={styles.spacer({ class: clsx(theme?.spacer, classes?.spacer) })}
		style={`height:${totalHeight}px;`}
	>
		<div
			class={styles.content({ class: clsx(theme?.content, classes?.content) })}
...
## Type
[VirtualListProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L2049)
## Props
@prop items = []
