<script lang="ts">
	...
	import { cardPlaceholder } from './theme.js';
	import type { CardPlaceholderProps } from '$lib/types.js';
...
<div role="status" {...restProps} class={base({ class: clsx(theme?.base, className) })}>
	<div class={area({ class: clsx(theme?.area, classes?.area) })}>
		<svg
			width="48"
			height="48"
			class={icon({ class: clsx(theme?.icon) })}
	...
	<div class={footer({ class: clsx(theme?.footer, classes?.footer) })}>
		<svg
			class={line({ class: clsx('h-14 w-14', theme?.line, classes?.line) })}
			aria-hidden="true"
			fill="currentColor"
			viewBox="0 0 20 20"
			xmlns="http://www.w3.org/2000/svg"
