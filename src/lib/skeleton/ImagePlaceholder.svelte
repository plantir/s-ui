<script lang="ts">
	...
	import { imagePlaceholder } from './theme.js';
	import type { ImagePlaceholderProps } from '$lib/types.js';
...
<div role="status" {...restProps} class={base({ class: clsx(className, theme?.base) })}>
	<div class={image({ class: clsx(theme?.image, classes?.image) })}>
		<svg
			width="48"
			height="48"
			class={svg({ class: clsx(theme?.svg, classes?.svg) })}
...
## Type
[ImagePlaceholderProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1540)
## Props
@prop size = "md"
@prop rounded
@prop imgOnly = false
