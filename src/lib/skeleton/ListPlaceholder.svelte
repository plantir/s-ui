<script lang="ts">
	...
	import { listPlaceholder } from './theme.js';
	import type { ListPlaceholderProps } from '$lib/types.js';
	...
	let {
		...
		classes,
		...restProps
	}: ListPlaceholderProps = $props();

	const theme = $derived(getTheme('listPlaceholder'));

	const { base, item, content, title, subTitle, extra } = $derived(
...
## Type
[ListPlaceholderProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1546)
## Props
@prop itemNumber = 5
@prop size = "md"
@prop rounded
