<script lang="ts">
	import type { SidebarContextType, SidebarProps } from '$lib/types.js';
	...
	let isLargeScreen = $derived(
		disableBreakpoints ? false : alwaysOpen || innerWidth >= breakpointValues[breakpoint]
	);
	...
	{#if isOpen || isLargeScreen}
		{#if isOpen && !alwaysOpen}
			...
			{:else if !backdrop && activateClickOutside}
				<div
					role="presentation"
					class="fixed inset-s-0 top-0 z-50 h-full w-full"
					onclick={closeSidebar}
				></div>
			{:else if !backdrop && !activateClickOutside}
				<div role="presentation" class="fixed inset-s-0 top-0 z-50 h-full w-full"></div>
			{/if}
		{/if}
		<aside
			use:trapFocus={!isLargeScreen && isOpen && !alwaysOpen
				? { onEscape: closeSidebar ? handleEscape : undefined }
				: null}
