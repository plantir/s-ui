<script lang="ts">
	...
	onMount(async () => {
		...
		try {
						...
						(typeof entry === 'object' &&
							...
							(typeof (entry as { colors?: unknown }).colors === 'undefined' ||
								(Array.isArray((entry as { colors?: unknown }).colors) &&
									(entry as { colors: unknown[] }).colors.every(
			...
			staticThemes = Array.from(
				new Map(themes.map((theme) => [theme.id, theme] as const)).values()
			);
		} catch (e) {
...
<div data-scope="theme-selector" data-part="base" {...restProps}>
	<Button
		data-part="button"
		color="gray"
		class={button({ class: clsx(themeStyling?.button, styling?.button) })}
		aria-haspopup="true"
	...
	<Dropdown
		data-part="dropdown"
		class={dropdown({ class: clsx(themeStyling?.dropdown, styling?.dropdown) })}
		bind:isOpen
		simple
	>
		{#each displayThemes as theme (theme.id)}
			<DropdownItem
				...
				class={item({
					...
					class: clsx(themeStyling?.item, styling?.item)
				})}
				role="option"
			...
			>
				<span
					...
					class={itemLabel({ class: clsx(themeStyling?.itemLabel, styling?.itemLabel) })}
					>{theme.name}</span
				>
				<div
					data-part="color-swatch-container"
					class={colorSwatchContainer({
						class: clsx(themeStyling?.colorSwatchContainer, styling?.colorSwatchContainer)
					})}
				>
					{#each theme.colors as color, index (`${theme.id}-${index}`)}
						<div
							...
							class={colorSwatch({
								swatchPosition:
									index === 0 ? 'first' : index === theme.colors.length - 1 ? 'last' : 'middle',
...
<!--
  ...
  ## Type
  [ThemeSelectorProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L2221)
  ## Props
  @prop classes
