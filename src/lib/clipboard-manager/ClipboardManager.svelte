<script lang="ts">
	import { Badge, Modal } from '$lib/index';
...
{#snippet inputArea()}
	<div class={styles.inputSection({ class: clsx(theme?.inputSection, classes?.inputSection) })}>
		...
		{#if items.length > 0}
			<div
				class={styles.searchWrapper({ class: clsx(theme?.searchWrapper, classes?.searchWrapper) })}
			>
				<div
				...
				>
					...
					/>
					<svg
						class={styles.searchIcon({ class: clsx(theme?.searchIcon, classes?.searchIcon) })}
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
					>
				...
				<button
					onclick={clearAll}
					class={styles.clearAll({ class: clsx(theme?.clearAll, classes?.clearAll) })}
				>
					{clearLabel}
				</button>
			</div>
		...
		{#if filteredItems.length === 0}
			...
			{:else}
				<div class={styles.emptyState({ class: clsx(theme?.emptyState, classes?.emptyState) })}>
					{#if items.length === 0}
						<svg
							class={styles.emptyIcon({ class: clsx(theme?.emptyIcon, classes?.emptyIcon) })}
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
						...
						<p class={styles.emptyText({ class: clsx(theme?.emptyText, classes?.emptyText) })}>
							No clipboard items yet.
						</p>
						<p
							class={styles.emptySubtext({
								class: clsx(theme?.emptySubtext, classes?.emptySubtext)
							})}
						>
			...
			{#each filteredItems as item (item.id)}
				...
				{:else}
					<div class={styles.item({ class: clsx(theme?.item, classes?.item) })}>
						<!-- Content -->
						<div
							class={styles.itemContent({ class: clsx(theme?.itemContent, classes?.itemContent) })}
						>
							<div
								class={styles.itemHeader({ class: clsx(theme?.itemHeader, classes?.itemHeader) })}
							>
								{#if item.pinned}
									<Badge {...badgeProps}>Pinned</Badge>
								{/if}
								<span
									class={styles.itemTimestamp({
										class: clsx(theme?.itemTimestamp, classes?.itemTimestamp)
									})}
								>
						...
						<!-- Actions -->
						<div
							class={styles.itemActions({ class: clsx(theme?.itemActions, classes?.itemActions) })}
						>
							<!-- Copy -->
							<button
								onclick={() => copyItem(item)}
								class={styles.actionButton({
									class: clsx(theme?.actionButton, classes?.actionButton)
								})}
								aria-label="Copy"
							>
								<svg
							...
							<button
								...
								class={styles.deleteButton({
									class: clsx(theme?.deleteButton, classes?.deleteButton)
								})}
								aria-label="Delete"
							>
								<svg
...
{#if selectionMenu.show}
	<div
		class={styles.selectionMenu({ class: clsx(theme?.selectionMenu, classes?.selectionMenu) })}
		style="left: {selectionMenu.x}px; top: {selectionMenu.y}px;"
	>
		<div
			class={styles.selectionBubble({
				class: clsx(theme?.selectionBubble, classes?.selectionBubble)
			})}
		>
			<span
				class={styles.selectionText({ class: clsx(theme?.selectionText, classes?.selectionText) })}
			>
		...
		<!-- Arrow -->
		<div
			class={styles.selectionArrow({ class: clsx(theme?.selectionArrow, classes?.selectionArrow) })}
		></div>
	</div>
{/if}
...
{#if toast}
	<div
		class={styles.toastContainer({ class: clsx(theme?.toastContainer, classes?.toastContainer) })}
	>
		<div class={styles.toast({ type: toast.type, class: clsx(theme?.toast, classes?.toast) })}>
			<svg
				class={styles.toastIcon({ class: clsx(theme?.toastIcon, classes?.toastIcon) })}
				fill="none"
				stroke="currentColor"
				stroke-width="2"
				viewBox="0 0 24 24"
			...
			<span class={styles.toastText({ class: clsx(theme?.toastText, classes?.toastText) })}
				>{toast.message}</span
			>
		</div>
	</div>
