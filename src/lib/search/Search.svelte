<script lang="ts">
	...
	import type { SearchProps } from '$lib/types.js';
	...
	const styling = $derived(
		classes ?? { input: inputClass, svg: clearableSvgClass, close: clearableClass }
	);
...
<div class={base({ class: clsx(theme?.base, className) })}>
	<div class={left({ class: clsx(theme?.left, classes?.left) })}>
		<svg
			class={icon({ class: clsx(theme?.icon, classes?.icon) })}
			aria-hidden="true"
	...
	<input
		...
		class={inputCls({ class: clsx(theme?.input, styling.input) })}
		{placeholder}
		required
		{...restProps}
	/>
...
## Type
[SearchProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L940)
## Props
@prop children
@prop inputClass
@prop size
