<script lang="ts">
	...
	const {
		...
		glow: stepGlow,
		incomplete
	} = $derived(stepIndicator({ size, color, glow, hideLabel }));
...
<div {...restProps} class={base({ class: clsx((theme as StepIndicatorTheme)?.base, className) })}>
	{#if !hideLabel}
		<h3 class={label({ class: clsx((theme as StepIndicatorTheme)?.label, classes?.label) })}>
			{currentStepLabel}
		</h3>
	{/if}

	<div
		class={container({ class: clsx((theme as StepIndicatorTheme)?.container, classes?.container) })}
	>
		{#each steps as _step, i (i)}
			{#if clickable}
				{#if i === currentStep - 1}
					<button
						...
						class={wrapper({
							class: clsx(
								(theme as StepIndicatorTheme)?.wrapper,
								classes?.wrapper,
						...
						{#if glow}
							<div
								class={stepGlow({
									class: clsx(
										getCustomStepClass(i),
										(theme as StepIndicatorTheme)?.glow,
										classes?.glow
									)
								})}
							></div>
				...
				{:else}
					<button
						...
						class={incomplete({
							class: clsx(
								(theme as StepIndicatorTheme)?.incomplete,
								classes?.incomplete,
								'cursor-pointer transition-opacity hover:opacity-75'
							)
						})}
			...
			{:else if i === currentStep - 1}
				<div
					class={wrapper({ class: clsx((theme as StepIndicatorTheme)?.wrapper, classes?.wrapper) })}
				>
					<div
						class={stepCls({
							class: clsx(
			...
			{:else}
				<div
					class={incomplete({
						class: clsx((theme as StepIndicatorTheme)?.incomplete, classes?.incomplete)
					})}
					data-state="incomplete"
				></div>
			{/if}
...
## Type
[StepIndicatorProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1632)
## Props
