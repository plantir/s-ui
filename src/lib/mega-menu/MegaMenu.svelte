<script lang="ts">
	import { megamenu } from './theme.js';
	import clsx from 'clsx';
	import type { MegaMenuProps } from '$lib/types.js';
...
>
	<div class={div({ class: clsx(theme?.div, classes?.div) })}>
		...
		{#if full && extra}<div class={extraCls({ class: clsx(theme?.extra, styling.extra) })}>
				{@render extra()}
			</div>{/if}
	</div>
</Popper>
...
## Type
[MegaMenuProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1151)
## Props
@prop children
@prop extra
@prop items = []
