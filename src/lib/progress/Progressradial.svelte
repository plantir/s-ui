<script lang="ts">
	import { progressradial } from './theme.js';
	import type { ProgressradialProps } from '$lib/types.js';
...
<div class="flex flex-col items-center">
	{#if labelOutside}
		<div class={outside({ class: clsx(theme?.outside, classes?.outside) })}>
			...
			<span class={progressCls({ class: clsx(theme?.progressCls, classes?.progressCls) })}
	...
	<div {...restProps} class={base({ class: clsx(size, theme?.base, className) })}>
		<svg viewBox="0 0 100 100" class="h-full w-full" style="transform: rotate({rotationAngle}deg)">
			...
			<circle
				...
				cy="50"
				r={radius}
				class={background({ class: clsx(theme?.background, classes?.background) })}
				fill="none"
...
## Type
[ProgressradialProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L1327)
## Props
@prop progress = 45
@prop radius = 42
