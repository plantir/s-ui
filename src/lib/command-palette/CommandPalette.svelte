<script lang="ts">
	...
	const filteredItems = $derived.by(() => {
		...
		return items.filter(
			(item) => check(item.label) || check(item.description) || item.keywords?.some(check)
		);
	});
...
## Type
[CommandPaletteProps](https://github.com/themesberg/s-ui/blob/main/src/lib/types.ts#L2313)
## Props
