<script lang="ts">
	import type { KanbanBoardProps, KanbanCardType } from '$lib/types.js';
	import clsx from 'clsx';
	import { kanbanBoard } from './theme.js';
...
<div {...restProps} class={styles.container({ class: clsx(theme?.container, className) })}>
	{#each columns as col (col.id)}
		...
		>
			<h2 class={styles.columnTitle({ class: clsx(theme?.columnTitle, classes?.columnTitle) })}>
				{col.title}
			</h2>
			...
			<button
				...
				aria-label={`Add card to ${col.title}`}>+ Add card</button
			>
		</div>
	{/each}
</div>
